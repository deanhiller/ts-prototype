...
function updateRedirectUrl(user: PartialDeep<UserData>) {
	/*
 ...
 */
export const setUser = createAsyncThunk<UserData, UserData>('user/setUser', async (user) => {
 ...
 */
const initialState: UserData = userModel({});
...
export const userSlice = createSlice({
	...
	reducers: {
		...
		setUserShortcuts: (state, action) => {
			...
			const newUser = _.setIn(oldState, 'data.shortcuts', action.payload) as UserData;
		...
		setUserSettings: (state, action) => {
			const oldState = _.cloneDeep(state);
			const newUser = _.setIn(oldState, 'data.settings', action.payload) as UserData;
		...
		updateUser: (state, action) => {
			const oldState = _.cloneDeep(state);
			const user = action.payload as PartialDeep<UserData>;
